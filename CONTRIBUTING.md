...
(`sessionforge-cli`, the standalone engine) and the repo root (the Paseo plugin, which depends on it). See
...
npm install   # sets up the workspace and builds packages/cli's dist/ automatically (a postinstall hook —
              # the root package's imports from "sessionforge-cli" only resolve once that build exists)
```
...
the root's `import ... from "sessionforge-cli"` resolves against `packages/cli/dist/`, which is git-ignored
