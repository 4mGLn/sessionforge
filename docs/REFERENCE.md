...
Platform support), and CI + an independently-usable CLI package (§19's "be installable" — now true two
ways: `paseo plugin install`, or the standalone binary via `install.sh`/`install.ps1`; not published to
npm).
