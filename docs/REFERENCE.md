...
"sessionforge-cli"`) resolves through completely standard `node_modules` resolution — no assumption that
...
true two ways: `paseo plugin install`, or `npm install -g sessionforge-cli`).
